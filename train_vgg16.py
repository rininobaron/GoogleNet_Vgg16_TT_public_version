...
L1 = 0
L2 = 255 #tranformación a imagen digital de 16 bits
...
x_train[:,:,:,0:4]=((L2-L1)/(T2-T1))*(x_train[:,:,:,0:4]-((T2+T1)/2))+(L2+L1)/2
...
x_train[:,:,:,4:]=((L2-L1)/(T2-T1))*(x_train[:,:,:,4:]-((T2+T1)/2))+(L2+L1)/2
...
x_test[:,:,:,0:4]=((L2-L1)/(T2-T1))*(x_test[:,:,:,0:4]-((T2+T1)/2))+(L2+L1)/2
...
x_test[:,:,:,4:]=((L2-L1)/(T2-T1))*(x_test[:,:,:,4:]-((T2+T1)/2))+(L2+L1)/2
...
model = VGG16(width=x_train.shape[2], height=x_train.shape[1], depth=8, NB_CLASS=CLASS_NUM)
opt = Adam(lr=INIT_LR, decay=INIT_LR / EPOCHS)
adagrad = Adagrad(lr=0.001, epsilon=1e-08)
rms = RMSprop(learning_rate=0.001, rho=0.9)
sdg = SGD(learning_rate=0.01, momentum=0.01, nesterov=False)
